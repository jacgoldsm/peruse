...
```
What if we want to normalize all these columns by the absolute frequency of the word in the data set? Normally, it would take a very long time to iterate over all 9,195 columns. However, it is very fast with a `hash_df`:
```{r}
...
```{r}
hash_frequencies$mutate_all(~.x / sum(.x))
```
