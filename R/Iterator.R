...
#' `$initial` as long as they are defined in the search path of the environment
#' where `yield_*` is called from. These values will not vary from iteration to
...
Iterator <- function(result,
  ...
  if (length(result) != 1L || !is.character(result)) {
    rlang::abort("`result` must be a character string of length 1")
  }
  result <- rlang::parse_exprs(result)

  if (!rlang::is_dictionaryish(initial)) {
    rlang::abort("`initial` must be a vector or list where every element has a valid name")
  }
